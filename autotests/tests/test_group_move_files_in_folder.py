...
from autotests.tests.conftest import create_folder, delete_file, group_move_files_in_folder

FOLDER: str = 'TestFolder123'
...
@allure.title('Проверка группового перемещения файлов')
@pytest.mark.usefixtures('authorization')
def test_group_move_files_in_folder(driver: WebDriver) -> None:
    disk_page = DiskPage(driver)
    disk_left_panel = DiskLeftPanel(driver)

    create_folder(driver=driver, name_folder=FOLDER)

    group_move_files_in_folder(driver=driver, names_files=FILES, name_folder=FOLDER)
    ...
    group_move_files_in_folder(driver=driver, names_files=FILES, name_folder='Файлы')
